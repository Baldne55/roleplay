/**
 * Scoped logger. Mirrors lc-rp's `Log.Info(...)` / `Log.Error(...)` pattern
 * but with a per-module scope tag so console output is greppable.
 *
 * Usage:
...
function Format(Level: LogLevel, Scope: string, Message: string, Extra?: unknown): string {
  ...
  const Base = `[${Stamp}] [${Level.toUpperCase()}] [${Scope}] ${Message}`;
